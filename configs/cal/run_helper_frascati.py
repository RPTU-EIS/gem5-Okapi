...
work_root = os.getcwd()
gem5_root = f"/import/home/schmitz/gem5-private-HW-domain"
gem5 = f"{gem5_root}/build/X86_Frascati/gem5.opt"
...
def run_benchmark(bench, bname, iteration, index, config, scheme, ap):
    ...
    run_ref = f"{gem5} -r {cal}/{config} {args}"
    print(run_ref)
...
def run_sim_benchmark(bench, bname, iteration, index, scheme, ap, s_name=""):
    ...
    for x in range(num_sims):
        redirect = f"-r --outdir={bname}_{iteration}_{x}_out"
        run_ref = f"{gem5} {redirect} {run_sim} {args} --sim_num {x}"
        print(run_ref)
        print(f"Finished with code {os.system(run_ref)}")
...
def main():
    ...
    tdir = f"{rdir}/{tag}"
    wdir = f"{gem5_root}/jobs/{name if not special else f'{name}_{s_name}'}"
